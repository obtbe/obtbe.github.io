...
For a 30-year-old, calculate $-3.5 + 0.08 * 30 = -1.1$, so $e^{-(-1.1)} = e^{1.1} \approx 3.004$. Then, $P(\text{Buy Popcorn}) = 1 / (1 + 3.004) \approx 0.25$, or a 25% chance. If the probability is above 0.5, predict “yes”; otherwise, “no”. The math adjusts b0 and b1 to **maximize the chance** the model matches actual yes-or-no choices.
...
A plant watered 3 days has a probability of $1 / (1 + e^{-(-4 + 0.9 * 3)}) \approx 0.43$, or 43%, so predict “no”. Logistic regression fits yes-or-no outcomes like these.
